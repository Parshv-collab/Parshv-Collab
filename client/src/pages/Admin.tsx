...
import { getPortfolioPalette, portfolioPalettes, type PortfolioPaletteId } from "@shared/palettes";
...
const paletteOptions = Object.values(portfolioPalettes);
...
function AdminStudio({ onExit }: { onExit: () => void }) {
  ...
  const setPalette = (paletteId: PortfolioPaletteId) => {
    const palette = getPortfolioPalette(paletteId);
    setDraft(current => ({ ...current, site: { ...current.site, palette: palette.id, accent: palette.accent } }));
    toast.success(`${palette.name} selected. Save changes to publish it.`);
  };
        ...
        <div className="grid gap-6 xl:grid-cols-[1.35fr_.65fr]">
          <div className="space-y-6">
            <section className="admin-card">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2"><p className="form-label">Site palette</p><p className="mt-2 text-sm leading-6 text-white/50">Select a complete background, text, and accent system. The palette is staged here and applied site-wide when you save.</p><div className="mt-4 grid gap-3 sm:grid-cols-2">{paletteOptions.map(palette => <button key={palette.id} type="button" onClick={() => setPalette(palette.id)} aria-pressed={draft.site.palette === palette.id} className={`focus-ring rounded-xl border p-3 text-left transition-colors ${draft.site.palette === palette.id ? "border-[#b8ff5c] bg-white/[.07]" : "border-white/10 bg-black/20 hover:border-white/30"}`}><span className="flex items-center justify-between gap-3"><span><strong className="block text-sm text-white">{palette.name}</strong><span className="mt-1 block text-xs text-white/50">{palette.description}</span></span><span className="flex shrink-0 overflow-hidden rounded-full border border-white/15" aria-hidden="true"><i className="h-5 w-5" style={{ backgroundColor: palette.background }} /><i className="h-5 w-5" style={{ backgroundColor: palette.foreground }} /><i className="h-5 w-5" style={{ backgroundColor: palette.accent }} /></span></span><span className="mt-3 block font-mono text-[10px] tracking-[.08em] text-white/40">{palette.background} · {palette.foreground} · {palette.accent}</span></button>)}</div></div>
              </div>
            ...
            <p className="-mt-2 text-sm leading-6 text-white/45">Projects switched off from the primary live-site selection remain public in the compact <span className="text-white/70">Other Projects</span> list below Selected Work.</p>
          ...
          <aside className="space-y-6 xl:sticky xl:top-6 xl:self-start">
            ...
            <section className="admin-card">
              ...
              <div className="preview-card" style={{ "--preview-accent": draft.site.accent, "--preview-bg": getPortfolioPalette(draft.site.palette).background, "--preview-fg": getPortfolioPalette(draft.site.palette).foreground } as CSSProperties}>
