...
export function WritingIndex() {
  ...
  return <><main className="min-h-screen bg-[#09090b] px-5 py-8 text-[#f5f5f5] sm:px-6 sm:py-10 md:px-10"><div className="mx-auto max-w-5xl"><Link href="/" className="focus-ring inline-flex min-h-11 items-center gap-2 font-mono text-xs text-white/55 hover:text-[var(--portfolio-accent)]"><ArrowLeft className="h-4 w-4" />Signal Atelier</Link><p className="eyebrow mt-12 sm:mt-14">WRITING / NOTES</p><h1 className="mt-5 text-[clamp(3.1rem,15vw,6rem)] font-black leading-[.92] tracking-[-.08em]">Thoughts in progress.</h1>{content.isError ? <div role="alert" className="mt-12 rounded-2xl border border-amber-400/30 bg-amber-400/10 p-8 text-amber-100">Writing is temporarily unavailable. Please refresh in a moment.</div> : <><div className="work-filter-strip mt-10 flex flex-nowrap gap-2 overflow-x-auto pb-1 sm:flex-wrap sm:overflow-visible">{tags.map(item => <button key={item} onClick={() => setTag(item)} className={`focus-ring min-h-10 shrink-0 rounded-full border px-3 py-1.5 text-xs ${tag === item ? "border-[var(--portfolio-accent)] bg-[var(--portfolio-accent)] text-black" : "border-white/15 text-white/55"}`}>{item}</button>)}</div>{content.isLoading ? <div className="mt-12 grid gap-4">{[1, 2, 3].map(item => <div key={item} className="h-32 animate-pulse rounded-2xl border border-white/10 bg-white/[.03]" />)}</div> : posts.length ? <div className="mt-12 divide-y divide-white/10">{posts.map(post => <Link key={post.id} href={`/writing/${post.slug}`} className="focus-ring group flex min-h-12 flex-col gap-5 py-7 md:flex-row md:items-center md:justify-between"><div><p className="font-mono text-[10px] uppercase tracking-[.14em] text-[var(--portfolio-accent)]">{new Date(post.publishedAt).toLocaleDateString()} · {readingTime(post.body)} min read</p><h2 className="mt-3 text-3xl font-bold tracking-[-.055em] group-hover:text-[var(--portfolio-accent)]">{post.title}</h2><p className="mt-3 max-w-2xl leading-7 text-white/55">{post.excerpt}</p></div><ArrowRight className="h-5 w-5 text-white/35 transition-transform group-hover:translate-x-1 group-hover:text-[var(--portfolio-accent)]" /></Link>)}</div> : <div className="mt-12 rounded-2xl border border-white/10 bg-white/[.025] p-6 text-white/50 sm:p-8">No writing has been published yet. Add a post in Content Studio when you are ready.</div>}</>}</div></main><BackToTop /></>;
}
